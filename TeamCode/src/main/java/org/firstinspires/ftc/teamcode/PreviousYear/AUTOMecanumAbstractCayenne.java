...
public abstract class AUTOMecanumAbstractCayenne extends LinearOpMode implements SensorEventListener {
    // Constants
    ...
    final static double JEWEL_FLICKER_MID = 0.39;
    final static double JEWEL_ARM_IN = 0.90;
    final static double JEWEL_GATE_CLOSED=0.7;
    final static double JEWEL_GATE_OPEN=0;

    final static double RELIC_ARM_START = 0.58+.029;
    final static double RELIC_ARM_GRAB = 0.545+.029;
    final static double RELIC_ARM_HOLD = 0.47+.035;
    final static double RELIC_ARM_PRE_GRAB = 0.539+.029;
    ...
    protected void initializeServos() {//initialization of servo positions
        ...
        servoGlyphRightBot.setPosition(GLYPH_GRIPPER_RIGHT_BOT_OPEN);
        servoGlyphLeftBot.setPosition(GLYPH_GRIPPER_LEFT_BOT_OPEN);
        servoGlyphLeftTop.setPosition(GLYPH_GRIPPER_LEFT_TOP_OPEN);
        servoGlyphRightTop.setPosition(GLYPH_GRIPPER_RIGHT_TOP_OPEN);
        servoRelicHand.setPosition(RELIC_HAND_CLOSED);
        servoRelicArm.setPosition(RELIC_ARM_START);
        servoGlyphSpinner.setPosition(SPINNER_DOUBLE_TOP);
        servoJewelGate.setPosition(JEWEL_GATE_CLOSED);
    }

    protected void releaseGlyph() {
        servoGlyphRightBot.setPosition(GLYPH_GRIPPER_RIGHT_BOT_MID);
        servoGlyphLeftBot.setPosition(GLYPH_GRIPPER_LEFT_BOT_MID);
        servoGlyphRightTop.setPosition(GLYPH_GRIPPER_RIGHT_TOP_MID);
        servoGlyphLeftTop.setPosition(GLYPH_GRIPPER_LEFT_TOP_MID);
    }

    protected void grabGlyph(){
        servoGlyphRightBot.setPosition(GLYPH_GRIPPER_RIGHT_BOT_CLOSED);
        servoGlyphLeftBot.setPosition(GLYPH_GRIPPER_LEFT_BOT_CLOSED);
        servoGlyphRightTop.setPosition(GLYPH_GRIPPER_RIGHT_TOP_CLOSED);
        servoGlyphLeftTop.setPosition(GLYPH_GRIPPER_LEFT_TOP_CLOSED);
        sleep(500);
    }
